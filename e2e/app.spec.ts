...
test.describe('詳細カードの操作', () => {
  ...
  test('ホバーで駅名ツールチップが表示され、訪問状態は変わらない @smoke', async ({ page }) => {
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('0／881駅'); // hoverでは変更しない
  });
  ...
  test('実ポインター操作: 詳細シートは色を変えず、シート内ボタンで青→赤→オレンジ→紫→青のフルサイクルとトーストの公式HP @smoke', async ({
    ...
    await page.getByTestId('btn-visited').click();
    await expectStage('#d83a34', 'rgb(216, 58, 52)', '1／881駅', '0');
    await page.getByTestId('btn-want').click();
    await expectStage('#d9640a', 'rgb(217, 100, 10)', '0／881駅', '0');
    await page.getByTestId('btn-stamp').click();
    await expectStage('#6a3ab2', 'rgb(106, 58, 178)', '1／881駅', '1');
    await page.getByTestId('btn-reset').click();
    await expectStage('#1a4f9e', 'rgb(26, 79, 158)', '0／881駅', '0');
    ...
    await page.waitForTimeout(500);
    await expect(page.getByTestId('stats-visited')).toContainText('1／881駅'); // 公式HPで状態は変わらない
...
test.describe('フィルターと達成率', () => {
  ...
  test('近畿追加後、北海道・東北・関東・北陸・中部・近畿の6地域が「地方」チップで自然に切替できる @smoke', async ({
    ...
    await expect(page.locator(`[data-sid="${KINKI_STATION}"]`)).toBeVisible();
    // 県フィルター変更に伴う地図の自動fitBoundsが収まるのを待つ
    // （この直後に__setMapView()しても、収まりきっていないfitBoundsが後から
    //   ビューを上書きし、固定UI（📍現在地/🔍周辺スポット）とマーカーが重なることがあった）
    await page.waitForTimeout(600);
  ...
  test('状態フィルターが排他状態と連動する（訪問済み⇔行きたいの移動）', async ({ page }) => {
    ...
    await page.getByTestId('filter-visited').click();
    await expect(page.locator(`[data-sid="${STATION_ID}"]`)).toHaveCount(0);
    await expect(page.getByTestId('stats-visited')).toContainText('0／881駅'); // 達成数から外れる
  });
  ...
  test('達成率100%の表示', async ({ page }) => {
    ...
    expect(ids && ids.length).toBe(881);
    await page.evaluate(
      ([key, idList]) => {
    ...
    await page.reload();
    await expect(page.getByTestId('stats-visited')).toContainText('881／881駅');
...
test.describe('ルート提案から旅行中まで', () => {
  ...
  test('シナリオA: 旅行中の1駅ナビ・到着=赤・スタンプ=紫・スキップ不変・帰路表示', async ({ page, context }) => {
    ...
    if (await page.getByTestId('trip-stamp').isVisible().catch(() => false)) {
      ...
      await expect(page.getByTestId('stats-visited')).toContainText('2／881駅');
      await expect(page.getByTestId('stats-stamped')).toContainText('1');
    }
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('2／881駅'); // スキップで状態不変
...
test.describe('全駅表示・駅名ラベル・地図の表示設定', () => {
  test('初期状態では収録全駅が881件の個別マーカーで表示され、まとめ表示（クラスタ）は出ない @smoke', async ({
    page,
  }) => {
    await page.goto('/');
    await closeLegend(page);
    await expect(page.locator('.rs-marker').first()).toBeVisible({ timeout: 15000 });
    await expect(page.locator('.rs-hit')).toHaveCount(881);
  ...
  test('「まとめて表示」に切り替えるとクラスタ表示になり、訪問記録は変化しない', async ({ page }) => {
    ...
    // 単独マーカーのまま出ることがあるため、881件全部ではないことだけ確認する）
    await expect(page.locator('.rs-hit')).not.toHaveCount(881);
    await expect(page.getByTestId('stats-visited')).toContainText('0／881駅');
    // 「全駅表示」へ戻すと元通りになる
