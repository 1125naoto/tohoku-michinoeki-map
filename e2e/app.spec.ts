...
async function openFilters(page: Page) {
  const chip = page.getByTestId('chip-all');
...
test.describe('地図と詳細カード @smoke', () => {
  test('地図が表示され、帰属表示・達成率ヘッダーがある', async ({ page }) => {
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('0／1080駅');
  ...
  test('地方・都道府県・表示フィルターがグループ分けされている @smoke', async ({ page }) => {
    ...
    await expect(page.getByRole('toolbar', { name: '地方で絞り込み' })).toBeVisible();
    await expect(page.getByRole('toolbar', { name: '都道府県で絞り込み' })).toBeVisible();
    await expect(page.getByRole('toolbar', { name: '表示状態で絞り込み' })).toBeVisible();
    await expect(page.locator('.fg-label').nth(0)).toHaveText('地方');
    await expect(page.locator('.fg-label').nth(1)).toHaveText('都道府県');
    await expect(page.locator('.fg-label').nth(2)).toHaveText('表示');
  });
...
test.describe('詳細カードの操作', () => {
  ...
  test('詳細カードから排他状態を直接設定できる（行きたい/訪問/スタンプ/未訪問）', async ({ page }) => {
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('1／1080駅');
  ...
  test('ホバーで駅名ツールチップが表示され、訪問状態は変わらない @smoke', async ({ page }) => {
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('0／1080駅'); // hoverでは変更しない
  });
  ...
  test('実ポインター操作: 詳細シートは色を変えず、シート内ボタンで青→赤→オレンジ→紫→青のフルサイクルとトーストの公式HP @smoke', async ({
    ...
    await page.getByTestId('btn-visited').click();
    await expectStage('#d83a34', 'rgb(216, 58, 52)', '1／1080駅', '0');
    await page.getByTestId('btn-want').click();
    await expectStage('#d9640a', 'rgb(217, 100, 10)', '0／1080駅', '0');
    await page.getByTestId('btn-stamp').click();
    await expectStage('#6a3ab2', 'rgb(106, 58, 178)', '1／1080駅', '1');
    await page.getByTestId('btn-reset').click();
    await expectStage('#1a4f9e', 'rgb(26, 79, 158)', '0／1080駅', '0');
    ...
    await page.waitForTimeout(500);
    await expect(page.getByTestId('stats-visited')).toContainText('1／1080駅'); // 公式HPで状態は変わらない
...
test.describe('フィルターと達成率', () => {
  ...
  test('北海道追加後、東北と混線せず共存する（県フィルター切替・地図・詳細表示）@smoke', async ({ page }) => {
    ...
    // 1. すべて: 北海道・東北の両方が地図に出る
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${AOMORI_STATION}"]`)).toBeVisible();

    // 2. 北海道チップ → 北海道の駅だけ表示、東北は消える
    await page.getByTestId('chip-北海道').click();
    await expect(page.getByTestId('chip-北海道')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${AOMORI_STATION}"]`)).toHaveCount(0);
  ...
  test('関東追加後、北海道・東北・関東の3地域が「地方」チップで自然に切替できる @smoke', async ({ page }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    const KANTO_STATION = 'mne-19149'; // 八王子滝山（東京都。関東で唯一の駅のため単独選択可能）
    ...
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toBeVisible();

    // 2. 「地方」チップ:関東 → 関東の駅だけ表示、北海道・東北は消える
    await page.getByTestId('chip-area-関東').click();
    await expect(page.getByTestId('chip-area-関東')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    ...
    // 4. 「地方」チップ:北海道 → 関東が消え、北海道だけになる
    await page.getByTestId('chip-area-北海道').click();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);

    // 5. 「地方」チップ:東北 → 東北6県だけになる
  ...
  test('北陸追加後、北海道・東北・関東・北陸の4地域が「地方」チップで自然に切替できる @smoke', async ({ page }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    const KANTO_STATION = 'mne-19149'; // 八王子滝山（東京都）
    const HOKURIKU_STATION = 'mne-19223'; // あいぽーと佐渡（新潟県佐渡市。最寄駅まで56km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:北陸 → 北陸の駅だけ表示、他3地域は消える
    await page.getByTestId('chip-area-北陸').click();
    await expect(page.getByTestId('chip-area-北陸')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${HOKURIKU_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
  ...
  test('中部追加後、北海道・東北・関東・北陸・中部の5地域が「地方」チップで自然に切替できる @smoke', async ({
    page,
  }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    const KANTO_STATION = 'mne-19149'; // 八王子滝山（東京都）
    const HOKURIKU_STATION = 'mne-19223'; // あいぽーと佐渡（新潟県佐渡市）
    const CHUBU_STATION = 'mne-19189'; // 小谷（長野県小谷村。最寄駅まで21.8km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:中部 → 中部の駅だけ表示、他4地域は消える
    await page.getByTestId('chip-area-中部').click();
    await expect(page.getByTestId('chip-area-中部')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${CHUBU_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
  ...
  test('近畿追加後、北海道・東北・関東・北陸・中部・近畿の6地域が「地方」チップで自然に切替できる @smoke', async ({
    page,
  }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    const KANTO_STATION = 'mne-19149'; // 八王子滝山（東京都）
    const HOKURIKU_STATION = 'mne-19223'; // あいぽーと佐渡（新潟県佐渡市）
    const CHUBU_STATION = 'mne-19189'; // 小谷（長野県小谷村）
    const KINKI_STATION = 'mne-19388'; // 伊勢志摩（三重県志摩市。最寄駅まで34.9km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:近畿 → 近畿の駅だけ表示、他5地域は消える
    await page.getByTestId('chip-area-近畿').click();
    await expect(page.getByTestId('chip-area-近畿')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${KINKI_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
    ...
    // ビューを上書きされ、固定UI（📍現在地/🔍周辺スポット）とマーカーが重なることがあった
    // （600ms程度では機材負荷時に間に合わないことがあった）。
    await page.waitForTimeout(2000);
  ...
  test('中国地方追加後、北海道・東北・関東・北陸・中部・近畿・中国の7地域が「地方」チップで自然に切替できる @smoke', async ({
    page,
  }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    ...
    const HOKURIKU_STATION = 'mne-19223'; // あいぽーと佐渡（新潟県佐渡市）
    const CHUBU_STATION = 'mne-19189'; // 小谷（長野県小谷村）
    const KINKI_STATION = 'mne-19388'; // 伊勢志摩（三重県志摩市）
    const CHUGOKU_STATION = 'mne-19560'; // みやま公園（岡山県玉野市。最寄駅まで26.6km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:中国 → 中国地方の駅だけ表示、他6地域は消える
    await page.getByTestId('chip-area-中国').click();
    await expect(page.getByTestId('chip-area-中国')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${CHUGOKU_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
    ...
    // 県フィルター変更に伴う地図の自動fitBoundsアニメーションが完全に収まるのを待つ。
    // アニメーション中に__setMapView()を呼ぶと、後から進行中のfitBoundsアニメーションに
    // ビューを上書きされることがある（600ms程度では機材負荷時に間に合わないことがあった）。
    await page.waitForTimeout(2000);
  ...
  test('四国追加後、北海道・東北・関東・北陸・中部・近畿・中国・四国の8地域が「地方」チップで自然に切替できる @smoke', async ({
    page,
  }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    ...
    const CHUBU_STATION = 'mne-19189'; // 小谷（長野県小谷村）
    const KINKI_STATION = 'mne-19388'; // 伊勢志摩（三重県志摩市）
    const CHUGOKU_STATION = 'mne-19560'; // みやま公園（岡山県玉野市）
    const SHIKOKU_STATION = 'mne-19664'; // 風早の郷風和里（愛媛県松山市。最寄駅まで25.5km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:四国 → 四国の駅だけ表示、他7地域は消える
    await page.getByTestId('chip-area-四国').click();
    await expect(page.getByTestId('chip-area-四国')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${SHIKOKU_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
  ...
  test('状態フィルターが排他状態と連動する（訪問済み⇔行きたいの移動）', async ({ page }) => {
    ...
    await page.getByTestId('filter-visited').click();
    await expect(page.locator(`[data-sid="${STATION_ID}"]`)).toHaveCount(0);
    await expect(page.getByTestId('stats-visited')).toContainText('0／1080駅'); // 達成数から外れる
  });

  test('県別グリッドの分母が自動計算されている @smoke', async ({ page }) => {
    await page.goto('/');
    await page.getByTestId('stats-toggle').click();
    await expect(page.getByTestId('pref-北海道')).toContainText('0／128駅');
    ...
    await expect(page.getByTestId('pref-茨城県')).toContainText('0／16駅');
    await expect(page.getByTestId('pref-栃木県')).toContainText('0／25駅');
    await expect(page.getByTestId('pref-群馬県')).toContainText('0／33駅');
    ...
    await expect(page.getByTestId('pref-福井県')).toContainText('0／21駅');
    await expect(page.getByTestId('pref-山梨県')).toContainText('0／22駅');
    await expect(page.getByTestId('pref-長野県')).toContainText('0／54駅');
    await expect(page.getByTestId('pref-岐阜県')).toContainText('0／55駅');
    await expect(page.getByTestId('pref-静岡県')).toContainText('0／28駅');
    await expect(page.getByTestId('pref-愛知県')).toContainText('0／19駅');
    await expect(page.getByTestId('pref-三重県')).toContainText('0／18駅');
    await expect(page.getByTestId('pref-滋賀県')).toContainText('0／20駅');
    await expect(page.getByTestId('pref-京都府')).toContainText('0／18駅');
    await expect(page.getByTestId('pref-大阪府')).toContainText('0／10駅');
    await expect(page.getByTestId('pref-兵庫県')).toContainText('0／37駅');
    await expect(page.getByTestId('pref-奈良県')).toContainText('0／18駅');
    await expect(page.getByTestId('pref-和歌山県')).toContainText('0／37駅');
    await expect(page.getByTestId('pref-鳥取県')).toContainText('0／17駅');
    await expect(page.getByTestId('pref-島根県')).toContainText('0／29駅');
    await expect(page.getByTestId('pref-岡山県')).toContainText('0／17駅');
    await expect(page.getByTestId('pref-広島県')).toContainText('0／21駅');
    await expect(page.getByTestId('pref-山口県')).toContainText('0／24駅');
    await expect(page.getByTestId('pref-徳島県')).toContainText('0／18駅');
    await expect(page.getByTestId('pref-香川県')).toContainText('0／18駅');
    await expect(page.getByTestId('pref-愛媛県')).toContainText('0／29駅');
    await expect(page.getByTestId('pref-高知県')).toContainText('0／26駅');
  });
  ...
  test('達成率100%の表示', async ({ page }) => {
    ...
    expect(ids && ids.length).toBe(1080);
    await page.evaluate(
      ([key, idList]) => {
    ...
    await page.reload();
    await expect(page.getByTestId('stats-visited')).toContainText('1080／1080駅');
...
test.describe('ルート提案から旅行中まで', () => {
  ...
  test('シナリオA: 旅行中の1駅ナビ・到着=赤・スタンプ=紫・スキップ不変・帰路表示', async ({ page, context }) => {
    ...
    if (await page.getByTestId('trip-stamp').isVisible().catch(() => false)) {
      ...
      await expect(page.getByTestId('stats-visited')).toContainText('2／1080駅');
      await expect(page.getByTestId('stats-stamped')).toContainText('1');
    }
    ...
    await expect(page.getByTestId('stats-visited')).toContainText('2／1080駅'); // スキップで状態不変
...
test.describe('スマホUI', () => {
  ...
  test('スマホでは絞り込みが初期折りたたみで、選択内容の1行サマリーが出る', async ({ page }) => {
    await page.goto('/');
    await closeLegend(page);
    // 初期は閉じており、サマリー1行表示（例: 絞り込み：すべて・すべて）
    await expect(page.getByTestId('chip-all')).toBeHidden();
    await expect(page.getByTestId('filters-toggle')).toContainText('絞り込み：すべて・すべて');
    // 展開→操作→たたむ
    await page.getByTestId('filters-toggle').click();
...
test.describe('地図全画面モード', () => {
  ...
  test('⛶全画面: ボタン表示→UI非表示→地図90%以上→中心維持→解除で復帰 @smoke', async ({ page }) => {
    ...
    // 1px未満の丸めは許容。全国化で収録駅の東西範囲が広がるほど初期表示のズームが
    // 下がり、1pxあたりの経度幅が大きくなるため、許容量も広域ズームに応じて緩める
    // （中国地方追加で北海道〜山口まで広がり、狭いビューポートでは0.02をわずかに超えた）
    expect(Math.abs(after.lat - before.lat)).toBeLessThan(0.03);
    expect(Math.abs(after.lng - before.lng)).toBeLessThan(0.03);
    // タイルが描画されている（灰色のみでない）
...
test.describe('全駅表示・駅名ラベル・地図の表示設定', () => {
  test('初期状態では収録全駅が1080件の個別マーカーで表示され、まとめ表示（クラスタ）は出ない @smoke', async ({
    page,
  }) => {
    await page.goto('/');
    await closeLegend(page);
    await expect(page.locator('.rs-marker').first()).toBeVisible({ timeout: 15000 });
    await expect(page.locator('.rs-hit')).toHaveCount(1080);
  ...
  test('「まとめて表示」に切り替えるとクラスタ表示になり、訪問記録は変化しない', async ({ page }) => {
    ...
    // 単独マーカーのまま出ることがあるため、1080件全部ではないことだけ確認する）
    await expect(page.locator('.rs-hit')).not.toHaveCount(1080);
    await expect(page.getByTestId('stats-visited')).toContainText('0／1080駅');
    // 「全駅表示」へ戻すと元通りになる
