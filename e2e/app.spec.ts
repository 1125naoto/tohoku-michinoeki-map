...
test.describe('フィルターと達成率', () => {
  ...
  test('九州追加後、北海道・東北・関東・北陸・中部・近畿・中国・四国・九州の9地域が「地方」チップで自然に切替できる @smoke', async ({
    page,
  }) => {
    const HOKKAIDO_STATION = 'mne-18899'; // わっかない（北海道）
    const TOHOKU_STATION = 'mne-18904'; // 青森県
    ...
    const KINKI_STATION = 'mne-19388'; // 伊勢志摩（三重県志摩市）
    const CHUGOKU_STATION = 'mne-19560'; // みやま公園（岡山県玉野市）
    const SHIKOKU_STATION = 'mne-19664'; // 風早の郷風和里（愛媛県松山市）
    const KYUSHU_STATION = 'mne-19704'; // むなかた（福岡県宗像市。最寄駅まで33.3km離れた孤立地点でクラスタ化しない）
    ...
    // 2. 「地方」チップ:九州 → 九州の駅だけ表示、他8地域は消える
    await page.getByTestId('chip-area-九州').click();
    await expect(page.getByTestId('chip-area-九州')).toHaveClass(/active/);
    await expect(page.locator(`[data-sid="${KYUSHU_STATION}"]`)).toBeVisible();
    await expect(page.locator(`[data-sid="${HOKKAIDO_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${TOHOKU_STATION}"]`)).toHaveCount(0);
    await expect(page.locator(`[data-sid="${KANTO_STATION}"]`)).toHaveCount(0);
  ...
  test('県別グリッドの分母が自動計算されている @smoke', async ({ page }) => {
    ...
    await expect(page.getByTestId('pref-福岡県')).toContainText('0／17駅');
    await expect(page.getByTestId('pref-佐賀県')).toContainText('0／11駅');
    await expect(page.getByTestId('pref-長崎県')).toContainText('0／12駅');
    await expect(page.getByTestId('pref-熊本県')).toContainText('0／38駅');
...
test.describe('地図全画面モード', () => {
  ...
  test('⛶全画面: ボタン表示→UI非表示→地図90%以上→中心維持→解除で復帰 @smoke', async ({ page }) => {
    ...
    // 数px未満の丸めは許容する。収録範囲が広がるほど初期表示のズームは下がり、
    // 1pxあたりの度数が大きくなるため、許容量は固定値ではなくズームから算出する
    // （地方を追加するたびに定数を上げ続けなくて済むようにした。中国追加時に0.02→0.03へ
    // 引き上げた経緯があり、九州追加で南端が27.8°Nまで伸びて0.03もわずかに超えたため）。
    // Webメルカトルでは 1px = 360 / (256 * 2^zoom) 度。中心維持の検証としては
    // 2px相当まで許容すれば十分で、実際に中心がずれる不具合（数十px以上）は検出できる。
    const degPerPx = 360 / (256 * 2 ** after.zoom);
    const allowed = degPerPx * 2;
    expect(Math.abs(after.lat - before.lat)).toBeLessThan(allowed);
    expect(Math.abs(after.lng - before.lng)).toBeLessThan(allowed);
    // タイルが描画されている（灰色のみでない）
