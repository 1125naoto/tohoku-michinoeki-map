...
import { evaluateSalesGate, renderMonitorSite, resolveSite } from './render';
...
describe('販売ゲート（evaluateSalesGate）', () => {
  it('リポジトリの実設定は、Ownerが確認するまで「受付準備中」（申込不可）のまま', () => {
    const gate = evaluateSalesGate(MONITOR_CONFIG, 'live');
    expect(gate.open).toBe(false);
    expect(gate.missing).toEqual(
      expect.arrayContaining(['販売事業者名', 'お問い合わせメール', '返金・キャンセル条件', 'Live Payment Link', 'Live Customer Portal']),
    );
  });

  it('リポジトリの実設定に個人情報・秘密値が入っていない（公開リポジトリ）', () => {
    const o = MONITOR_CONFIG.owner;
    for (const v of Object.values(o)) expect(v).toBeNull();
    expect(MONITOR_CONFIG.live.paymentLink).toBeNull();
    expect(MONITOR_CONFIG.live.portalLoginUrl).toBeNull();
    expect(JSON.stringify(MONITOR_CONFIG)).not.toMatch(/sk_(live|test)_|rk_(live|test)_|whsec_|@gmail\.com/);
  });

  it('必須項目・Live URLがすべて揃うと受付中になる', () => {
    expect(evaluateSalesGate(OPEN_CFG, 'live')).toEqual({ open: true, missing: [] });
  });
  ...
  it('testモードは Test URL＋ダミー事業者で受付中になる', () => {
    expect(evaluateSalesGate(MONITOR_CONFIG, 'test')).toEqual({ open: true, missing: [] });
  });
});
...
describe('受付準備中（本番ビルドの現状）', () => {
  ...
  it('未確認の事業者情報を捏造せず「受付開始時に掲載します」と出す', () => {
...
describe('受付中（Ownerの確認値とLive URLが揃った場合）', () => {
  ...
  it('特商法・規約・プライバシーにOwner確認値が入り、所在地・電話は請求開示の文言になる', () => {
    ...
    expect(text(html(files, 'terms'))).toContain('表示価格は税込みです。');
