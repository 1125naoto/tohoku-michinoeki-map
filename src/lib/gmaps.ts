...
/**
 * 周辺スポットパネルの外部探索導線の設計方針（Fable 5.1 Root Cause Audit
 * 再監査の最終形）。役割を3つに明確分離する:
 * (1) アプリ内POI＝道の駅周辺の候補をすぐ見る（OSM/Overpass由来。道の駅ナビの
 *     主目的である発見・車旅・ルート作成に沿う。Google Mapsと同等の店舗網羅性は
 *     構造上保証できないため断定しない）
 * (2) Google Web検索（src/lib/websearch.ts）＝より詳しく・網羅的に探す
 * (3) Google Maps（本ファイル）＝道の駅そのものの場所・口コミ・写真・営業時間・
 *     ナビを見る専用。「駅周辺のカテゴリ検索」はさせない
 *
 * 根本原因（Owner iPhone QA・道の駅たまかわ）: 以前は「キーワード + 生の緯度,経度」
 * を1つの自由テキストとして連結してGoogleへ渡していたが（例:
 * `飲食店 37.222832,140.418137`）、Maps URLs Search actionにはその座標を
 * 検索中心として扱う公式パラメータが存在せず、Googleが自由テキストとして
 * 解釈した結果、検索地点が実際の指定座標ではなく端末の現在地扱いになっていた。
 * 座標や駅名を挟んでも「指定した地点を検索中心に固定したままカテゴリ検索する」
 * こと自体がMaps URLs公式仕様では保証できないため、query調整による対処は
 * これ以上行わない（stationSearchUrl/latLngUrlという既存の正常系のみ使う）。
 */
